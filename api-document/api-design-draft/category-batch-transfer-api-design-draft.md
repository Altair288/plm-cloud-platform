...
- 属性复制同样只复制 latest version，不复制属性历史版本。
- 枚举属性绑定的 LOV 选项会一并复制，避免出现“分类复制成功但属性为空”的不完整状态。

补充说明：

- 这里的 COPY 不是只复制分类树节点本身，而是复制“分类节点 + 分类当前版本 + 分类下属性当前版本”。
- 因为属性和枚举值在当前模型下都存在业务域内唯一编码约束，所以属性编码、LOV 绑定编码、枚举值编码不会直接复用源对象编码。
- 复制出的属性会按当前规则重新生成属性编码；若属性为枚举型，则其 LOV 绑定编码和枚举值编码也按当前规则重新生成。
- 复制出的属性展示名、字段名、数据类型、布尔/数值配置、描述、枚举项 name/label 等业务内容保持与源 latest version 一致。
...
- 分类复制时，分类级 codeMappings 仍只返回分类 code 的 oldCode -> newCode 映射。
- 分类下属性不会复用原 attribute key，而是按目标复制分类的上下文重新生成 attribute key。
- 若属性为枚举型，不复用原 `lovKey`，而是按新属性重新生成内部 LOV 绑定编码。
- 枚举值 option code 也不会直接复用源值编码，而是重新生成，以满足同 businessDomain 下枚举值编码唯一约束。
- 复制后的枚举值 `name` / `label` 保持与源值一致，变化点仅在系统编码重新分配。
...
- 复制出的属性会跟随新分类落到新对象树下。
- 属性 latest version 从 1 开始重新建立，不沿用源属性版本号。
- 当前接口响应仍以分类维度返回结果，不额外返回 attribute key / lov key / enum option code 的映射；前端若需要查看复制后的属性结果，应继续通过属性查询接口按新分类 code 查询。
