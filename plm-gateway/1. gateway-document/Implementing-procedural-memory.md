...
- auth-service 当前对象存储接口前缀稳定为 /api/storage/**，适合直接按 Path=/api/storage/** 转发到 8081。
...
- 已为 plm-gateway 新增 GatewayRoutingConfigIT，使用 dev profile 校验 server.port=8080，以及 `/auth/**`、`/api/storage/**`、`/api/meta/**` 三条路由已正确装配。
